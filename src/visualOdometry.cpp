...
void imagePointsHandler(const sensor_msgs::PointCloud2ConstPtr& imagePoints2)
{
  ...
  bool nanWarnArray[ipRelationsNum];
  for (int i = 0; i < ipRelationsNum; i++)
    nanWarnArray[i] = false;
  ...
  for (int iterCount = 0; iterCount < iterNum; iterCount++) {
    ...
    for (int i = 0; i < ipRelationsNum; i++) {
      ...
      if (!std::isfinite(ipr.s)) {
        ROS_WARN_THROTTLE(nanWarnArray[i] == false, "Catching ipr.s = NaN issue at ipRelations->points[%d]", i);
        nanWarnArray[i] = true;
        ipRelations->points[i].v = -1;
      } else if (fabs(ipr.v) < 0.5) {
    ...
    if (ipRelations2Num > 10) {
      ...
      matAtA = matAt * matA;
      matAtB = matAt * matB;
      // This used to be QR decomposition, but returns NaNs on some machines.  I
      // switched this to SVD to prevent that from happening
      cv::solve(matAtA, matAtB, matX, cv::DECOMP_SVD);
...
{
  // Convert NED IMU Orientation to NWU
  double roll, pitch, yaw;
  tf::Quaternion NED_orientation;
  tf::quaternionMsgToTF(imuData->orientation, NED_orientation);
  tf::Quaternion NWU_orientation;
  NWU_orientation.setW(NED_orientation.getW());
  NWU_orientation.setX(NED_orientation.getX());
  NWU_orientation.setY(-NED_orientation.getY());
  NWU_orientation.setZ(-NED_orientation.getZ());
  tf::Matrix3x3(NWU_orientation).getRPY(roll, pitch, yaw);
...
void imageDataHandler(const sensor_msgs::Image::ConstPtr& imageData) 
{
  ...
  // ipr.v = -1 --> Marked for deletion next time through
  int ipRelationsNum = ipRelations->points.size();
  for (int i = 0; i < ipRelationsNum; i++) {
    if (fabs(ipRelations->points[i].v) < 0.5) {
      cv::circle(bridge->image, cv::Point((kImage[2] - ipRelations->points[i].z * kImage[0]) / showDSRate,
                (kImage[5] - ipRelations->points[i].h * kImage[4]) / showDSRate), 1, CV_RGB(255, 0, 0), 2); // ipr.v = 0 --> Red (no depth)
    } else if (fabs(ipRelations->points[i].v - 1) < 0.5) {
      cv::circle(bridge->image, cv::Point((kImage[2] - ipRelations->points[i].z * kImage[0]) / showDSRate,
                (kImage[5] - ipRelations->points[i].h * kImage[4]) / showDSRate), 1, CV_RGB(0, 255, 0), 2); // ipr.v = 1 --> Green (measured depth)
    } else if (fabs(ipRelations->points[i].v - 2) < 0.5) {
      cv::circle(bridge->image, cv::Point((kImage[2] - ipRelations->points[i].z * kImage[0]) / showDSRate,
                (kImage[5] - ipRelations->points[i].h * kImage[4]) / showDSRate), 1, CV_RGB(0, 0, 255), 2); // ipr.v = 2 --> Blue (estimated depth)
